...
public class GroundVehiclePathNavigation extends VehiclePathNavigation {
    ...
    protected PathFinder createPathFinder(int p_26453_) {
        this.nodeEvaluator = new GroundVehicleNodeEvaluator();
    ...
    protected boolean canUpdatePath() {
        return this.vehicle.onGround() || this.isInLiquid() || this.mob.isPassenger();
    }

    protected Vec3 getTempMobPos() {
        return new Vec3(this.vehicle.getX(), (double)this.getSurfaceY(), this.vehicle.getZ());
    }
    ...
    private int getSurfaceY() {
        if (this.vehicle.isInWater() && this.canFloat()) {
            ...
            return Mth.floor(this.vehicle.getY() + 0.5D);
        }
    }
    ...
    protected void trimPath() {
        super.trimPath();
        if (this.avoidSun) {
            if (this.level.canSeeSky(BlockPos.containing(this.vehicle.getX(), this.vehicle.getY() + 0.5D, this.vehicle.getZ()))) {
                return;
            }
