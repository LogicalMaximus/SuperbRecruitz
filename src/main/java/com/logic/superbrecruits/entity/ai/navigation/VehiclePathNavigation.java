...
public abstract class VehiclePathNavigation {
    ...
    protected Path createPath(Set<BlockPos> p_148223_, int p_148224_, boolean p_148225_, int p_148226_, float p_148227_) {
        if (p_148223_.isEmpty()) {
            ...
            BlockPos blockpos = p_148225_ ? this.vehicle.blockPosition().above() : this.vehicle.blockPosition();
    ...
    protected void followThePath() {
        Vec3 vec3 = this.getTempMobPos();
        this.maxDistanceToWaypoint = this.mob.getBbWidth() > 0.75F ? this.vehicle.getBbWidth() / 2.0F : 0.75F - this.vehicle.getBbWidth() / 2.0F;
        Vec3i vec3i = this.path.getNextNodePos();
        double d0 = Math.abs(this.vehicle.getX() - ((double)vec3i.getX() + (this.vehicle.getBbWidth() + 1) / 2D)); //Forge: Fix MC-94054
        double d1 = Math.abs(this.vehicle.getY() - (double)vec3i.getY());
        double d2 = Math.abs(this.vehicle.getZ() - ((double)vec3i.getZ() + (this.vehicle.getBbWidth() + 1) / 2D)); //Forge: Fix MC-94054
    ...
    protected void doStuckDetection(Vec3 p_26539_) {
        ...
        if (this.path != null && !this.path.isDone()) {
            ...
            if (vec3i.equals(this.timeoutCachedNode)) {
                ...
                this.timeoutLimit = 0.21F > 0.0F ? d0 / (double)0.21F * 20.0D : 0.0D;
            }
    ...
    public boolean shouldRecomputePath(BlockPos p_200904_) {
        if (this.hasDelayedRecomputation) {
            ...
            Vec3 vec3 = new Vec3(((double)node.x + this.vehicle.getX()) / 2.0D, ((double)node.y + this.vehicle.getY()) / 2.0D, ((double)node.z + this.vehicle.getZ()) / 2.0D);
